...
open class VolleyFileUploadRequest(method: Int, url: String, listener: Response.Listener<NetworkResponse>, errorListener: Response.ErrorListener): Request<NetworkResponse>(method, url, errorListener) {
    ...
    override fun getHeaders(): MutableMap<String, String> = when(headers) {
    ...
    private fun processParams(dataOutputStream: DataOutputStream, params: Map<String, String>, encoding: String) {
        try {
            params.forEach {
                ...
                dataOutputStream.write(it.value.toByteArray())
                dataOutputStream.writeBytes(ending)
            }
