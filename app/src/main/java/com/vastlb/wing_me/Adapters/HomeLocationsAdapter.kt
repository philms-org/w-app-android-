...
class HomeLocationsAdapter(val array: ArrayList<HomeLocationClass>, val select: (index: Int) -> Unit): RecyclerView.Adapter<HomeLocationsAdapter.ViewHolder>() {
    ...
    override fun onCreateViewHolder(p0: ViewGroup, p1: Int): ViewHolder {
        val cell = LayoutInflater.from(p0.context).inflate(R.layout.row_layout_home_location, p0, false)
        return ViewHolder(cell, p1)
    }

    override fun onBindViewHolder(p0: ViewHolder, p1: Int) {
        val jsonObject: HomeLocationClass = array[p1]

        Picasso.get().load(Constants.url + jsonObject.url).into(p0.imageView)

        p0.titleTextView.setText(jsonObject.name)
        p0.detailsTextView.setText(jsonObject.description)
