...
class CommentsAdapter(val context: Context, val array: ArrayList<CommentClass>, val select: (index: Int) -> Unit, val longClick: (index: Int) -> Unit, val openProfile: (index: Int) -> Unit, val like: (index: Int, like: Boolean) -> Unit): RecyclerView.Adapter<CommentsAdapter.ViewHolder>() {
    ...
    override fun onCreateViewHolder(p0: ViewGroup, p1: Int): ViewHolder {
        val cell = LayoutInflater.from(p0.context).inflate(R.layout.row_layout_comment, p0, false)
        return ViewHolder(cell, p1)
    }

    @SuppressLint("UseCompatLoadingForDrawables")
    override fun onBindViewHolder(p0: ViewHolder, p1: Int) {
        val jsonObject: CommentClass = array[p1]
